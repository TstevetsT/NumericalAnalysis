...
% Demonstrate the code is slow for large n
clear

f = @(x) sin(x);
z = linspace(-pi,pi,100); % These are the evaluation points

n = 1000;
x = chebyspace(-pi,pi,n+1);

tic
w = lagrange_weights(x);
disp('weight computation')
toc
...
toc
tic
g = lagrange_eval_barycentric(z,x,f(x),w);
disp('barycentric eval')
toc
